...
void test(void){
    ...
    c = csa_init();
    assert(!csa_get(c, 17, &n));
    assert(!csa_get(c, 64, &n));
    // First Add (csa[47]=100)
    assert(csa_set(c, 47, 42));
    // Add csa[64]=30
    assert(csa_set(c, 64, 30));
    // Add csa[64]=100
    assert(csa_set(c, 64, 100));

    // Getters
    assert(csa_get(c, 47, &n));
    assert(n==42);
    assert(csa_get(c, 64, &n));
    assert(n==100);
    assert(block_idx_get(*c, 64)==1);
    assert(block_idx_get(*c, -1)==-1);
    assert(block_idx_get(*c, 222)==-1);
    assert(val_idx_get(c->b[0], 47)==0);
    assert(block_vals_count(c->b[0])==1);
    assert(block_vals_count(c->b[1])==1);
    assert(insert_idx_get(*c, 128)==2);
    assert(insert_idx_get(*c, 191)==2);
    assert(block_create(c, 128, 777));
    assert(csa_get(c, 128, &n));
    assert(n==777);
    assert(block_update(&c->b[2], 128-(c->b[2].offset), 888));
    assert(csa_get(c, 128, &n));
    assert(n==888);
    assert(block_delete(c, 2));
    assert(!csa_get(c, 128, &n));
    assert(csa_set(c, 17, 24));
    assert(csa_get(c, 17, &n));
    assert(n==24);
    assert(val_delete(&c->b[0], 17-(c->b[0].offset), 2));
    csa_tostring(c, str);
    assert(strcmp(str, "2 blocks {1|[47]=42}{1|[64]=100}")==0);
    // Clean Up
    csa_free(&c);
    assert(c==NULL);
...
int block_idx_get(const csa c, int idx){
    ...
    for(int i=0; i<c.n; i++){
        if((unsigned int)idx >= c.b[i].offset && (unsigned int)idx < c.b[i].offset+MSKLEN){
            return i;
        }
    }
    return -1;
